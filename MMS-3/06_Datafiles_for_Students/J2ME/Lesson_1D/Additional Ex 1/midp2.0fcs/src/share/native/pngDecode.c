...
static jboolean handleImageData(unsigned char *, int, imageDstPtr, pngData *);
static unsigned long getInt(imageSrcPtr);
static unsigned long skip(imageSrcPtr, int, unsigned long);
static jboolean getChunk(imageSrcPtr, unsigned long *, long *);
static jboolean signatureOK(imageSrcPtr);
static jboolean headerOK(pngData *);
static unsigned long init_CRC(unsigned long);
static jboolean check_CRC(imageSrcPtr, unsigned long);
...
unsigned int
PNGdecodeImage_getBytes(unsigned char *buffer, int count, void *p)
{
    idatInflateData *d = (idatInflateData *)p;
    imageSrcPtr    src = d->src;
    unsigned long chunkType;
    ...
    count = src->getBytes(src, buffer, count);
    d->clen -= count;
    return count;
}
...
PNGdecodeImage_real(imageSrcPtr src, imageDstPtr dst, 
    ...
    jboolean       OK = KNI_TRUE;
    jboolean saw_IDAT = KNI_FALSE,
             saw_PLTE = KNI_FALSE,
             saw_tRNS = KNI_FALSE;
    pngData      data;
    ...
    while (getChunk(src, &chunkType, &chunkLength)) {
        ...
        if (chunkType == IHDR_CHUNK) {
            ...
            decompBuf = (unsigned char*)midpMalloc(decompLen);
            if (decompBuf == NULL) {
                OK = KNI_FALSE;
		goto done;
            }
	    ...
	    PNGdecodeImage_getByte(&_data);
            ...
            if (!midp_pngInflate(&_data, compLen - 4, &decompBuf, decompLen)) {
                freeBytes(decompBuf);
		goto formaterror;
	    }
        ...
        if (!check_CRC(src, CRC)) {
            /* fprintf(stderr, "PNG data corrupted (CRC mismatch)\n"); */
...
PNGdecodeImage(imageSrcPtr src, imageDstPtr dst) {
    ...
    paletteData = midpMalloc(sizeof(long) * 256);
    if (paletteData == NULL) {
	/* out of memory */
	return KNI_FALSE;
    } else {
	transData = midpMalloc(sizeof(unsigned char) * 256);
	if (transData == NULL) {
	    /* out of memory */
	    midpFree(paletteData);
	    return KNI_FALSE;
	} else {
	    jboolean retval = PNGdecodeImage_real(src, dst, 
...
*
*/
static char *pStartX = "\000\004\000\002\000\001\000\000";
static char *pIncX   = "\010\010\004\004\002\002\001\001";
/* pass number            1   2   3   4   5   6   7   - */
/* static char *pStartY = "\000\000\004\000\002\000\001\000"; */
/* static char *pIncY   = "\010\010\010\004\004\002\002\001"; */
