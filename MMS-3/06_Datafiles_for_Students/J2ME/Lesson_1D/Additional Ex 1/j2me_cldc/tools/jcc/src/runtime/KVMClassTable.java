...
public class KVMClassTable extends KVMHashtable { 
   ...
   private void addArrayClass(KVMClassName cn) {
    ...
    public NameAndTypeKey getNameAndTypeKey(String name, String type) {
        return getNameAndTypeKey(name, type, false);
    }

    public NameAndTypeKey getNameAndTypeKey(String name, String type, 
                                            boolean mayBeRenamed) {
        int nameKey = getNameKey(name, mayBeRenamed);
        int typeKey;
        if (type.charAt(0) == '(') { 
            typeKey = getNameKey(encodeMethodSignature(type), false);
        } else { 
    ...
    public int getNameKey(String name) { 
        return getNameKey(name, false);
    }
    public int getNameKey(String name, boolean mayBeRenamed) { 
        return nameTable.getNameKey(name, mayBeRenamed);
    }
