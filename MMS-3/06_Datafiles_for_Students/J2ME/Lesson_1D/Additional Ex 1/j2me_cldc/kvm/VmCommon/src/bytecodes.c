...
*********/
            ...
            default:
                /* RAISE_EXCEPTION(VerifyError); */
                fatalError(KVM_MSG_ILLEGAL_WIDE_BYTECODE_EXTENSION);
                break;
        }
...
SELECT(MULTIANEWARRAY)           /* Create new multidimensional array */
        ...
        VMSAVE
        result = instantiateMultiArray(thisClass, (long*)(sp_global - dimensions + 1), dimensions);
        VMRESTORE
        if (result != NULL) {
            sp -= dimensions;
